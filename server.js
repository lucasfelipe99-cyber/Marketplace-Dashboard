...
function resolvePublicFile(urlPath) {
  ...
  if (decodedPath.startsWith('/data/')) {
    ...
    return path.join(dataDir, requestedName);
  }
...
async function handleBaseUpload(request, response) {
  ...
  try {
    const size = await streamRequestToFile(request, storedPath, maxUploadBytes);
    const metadata = {
...
async function handleRowsUpload(request, response) {
  ...
  try {
    await streamRequestToFile(request, rowsPath, maxUploadBytes);
...
function readPublishedRows(monthMetadata) {
  ...
  const rowsPath = path.join(dataDir, monthMetadata.rowsName);
...
function finalizeMetricBucket(bucket) {
  const attributedRevenue = bucket.advertising > 0 ? bucket.revenue : 0;
  ...
  return {
    ...
    moas: bucket.gm - Math.abs(bucket.advertising),
    adsRevenueShare: bucket.revenue === 0 ? 0 : attributedRevenue / bucket.revenue,
    ads: bucket.ads.size
  };
}

function aggregatePublishedScenario(rows, month, year, scenarioKind) {
  ...
  const finalizeMap = (map, limit) => Array.from(map.values())
  ...
  return {
    ...
    totals: totalMetrics,
    marketplaces: finalizeMap(groups.marketplaces, 20),
    categories: finalizeMap(groups.categories, 20),
    skus: finalizeMap(groups.skus, 30),
    ads: finalizeMap(groups.ads, 30)
  };
}
...
function calculateAverageMetrics(months) {
  ...
  const numericKeys = [
    ...
    'advertising', 'tacos', 'moas', 'adsRevenueShare'
  ];
  const result = {};
...
function buildIntelligentAnalytics() {
  ...
  const months = monthEntries.map(([month, monthMetadata]) => {
    ...
    return aggregatePublishedMonth(rows, month, updatedYear);
  }).filter(Boolean);
  ...
  const average3 = calculateAverageMetrics(months.slice(-3));
  const average6 = calculateAverageMetrics(months.slice(-6));
  const comparisons = {};

  ['revenue', 'netRevenue', 'quantity', 'averageTicket', 'gm', 'gmPercent', 'advertising', 'tacos', 'moas'].forEach((key) => {
    comparisons[key] = {
      vsPrevious: calculateMetricVariation(current.totals[key], previous && previous.totals[key]),
      ...
      vsForecast: calculateMetricVariation(current.totals[key], current.forecastTotals && current.forecastTotals[key])
    };
  });
  ...
  return {
    ...
    current: Object.assign({}, current, {
      marketplaces: addTrendVariation(current.marketplaces, previous && previous.marketplaces),
      categories: addTrendVariation(current.categories, previous && previous.categories),
      skus: addTrendVariation(current.skus, previous && previous.skus),
      ads: addTrendVariation(current.ads, previous && previous.ads)
    }),
    previous,
    average3,
    average6,
    comparisons,
    abc: buildIntelligentAbcSummary(current.skus),
    risks: buildIntelligentRiskSummary(current)
  };
}
...
function getPublishedDataSignature() {
  ...
  return crypto.createHash('sha256').update(JSON.stringify(months)).digest('hex');
}
...
function readIntelligentAnalysisCache() {
  if (!fs.existsSync(intelligentAnalysisCachePath)) {
    return null;
  }

  try {
    return JSON.parse(fs.readFileSync(intelligentAnalysisCachePath, 'utf8'));
  } catch (error) {
...
function writeIntelligentAnalysisCache(result) {
  fs.writeFileSync(intelligentAnalysisCachePath, JSON.stringify(result, null, 2));
}
...
function compactIntelligentAnalytics(analytics) {
  ...
  return {
    ...
    monthlyEvolution: analytics.months.map((month) => ({
      month: month.label,
      totals: month.totals,
      marketplaces: month.marketplaces.slice(0, 8).map(compactMetric)
    })),
    ...
    marketplaces: analytics.current.marketplaces.slice(0, 12).map(compactMetric),
    categories: analytics.current.categories.slice(0, 15).map(compactMetric),
    skus: analytics.current.skus.slice(0, 20).map(compactMetric),
    ads: analytics.current.ads.slice(0, 20).map(compactMetric),
    goals: {
      source: 'Forecast da base carregada',
      totals: analytics.current.forecastTotals,
      marketplaces: analytics.current.forecastMarketplaces.slice(0, 12).map(compactMetric),
      categories: analytics.current.forecastCategories.slice(0, 15).map(compactMetric)
    },
    abc: analytics.abc,
...
function parseIntelligentStructuredAnalysis(text) {
  try {
    ...
    return {
      ...
      businessNarrative: String(parsed.businessNarrative || '').trim(),
      financialDiagnosis: String(parsed.financialDiagnosis || '').trim(),
      kpiAssessment: String(parsed.kpiAssessment || '').trim(),
      trends: mapItems(parsed.trends, 8),
      alerts: mapItems(parsed.alerts, 8),
      recommendations: mapItems(parsed.recommendations, 10)
    };
  } catch (error) {
    return null;
  }
...
async function generateIntelligentAnalysis(force) {
  ...
  if (!force && cached && cached.signature === analytics.generatedFrom && cached.analysis) {
  ...
  const openAiResponse = await fetch('https://api.openai.com/v1/responses', {
    ...
    body: JSON.stringify({
      ...
      instructions: [
        ...
        'Analise o mes atual contra o mes anterior, a media dos ultimos 3 meses e a media dos ultimos 6 meses quando houver historico suficiente.',
        ...
        'Considere faturamento bruto e liquido, quantidade, ticket medio, GM em reais e percentual, publicidade, TACOS, ACOS, ROAS, MOAS e participacao de receita dos anuncios.',
        ...
        'As recomendacoes devem dizer o que fazer, por que fazer, prioridade, responsavel sugerido, prazo e resultado esperado.',
        ...
        'Seja profundo sem ser prolixo: executiveSummary deve ter entre 450 e 700 palavras; financialDiagnosis, kpiAssessment e businessNarrative entre 250 e 450 palavras cada.',
  ...
  if (!analysis
      || analysis.executiveSummary.length < 800
      || analysis.businessNarrative.length < 450
      || analysis.financialDiagnosis.length < 450
      || analysis.kpiAssessment.length < 450
      || analysis.trends.length < 6
      || analysis.alerts.length < 6
      || analysis.recommendations.length < 8) {
