...
function scaleMetricTotals(totals, factor) {
  ...
  ['revenue', 'netRevenue', 'quantity', 'gm', 'advertising', 'moas'].forEach((key) => {
...
function scaleMetricItems(items, factor) {
  return (items || []).map((item) => Object.assign({}, item, {
    ...
    moas: (Number(item.moas) || 0) * factor
  }));
}
...
function compactIntelligentAnalytics(analytics) {
  ...
  return {
    ...
    monthlyEvolution: analytics.months.map((month) => ({
      month: month.label,
      totals: month.totals,
      marketplaces: month.marketplaces.slice(0, 3).map(compactMetric)
    })),
    ...
    marketplaces: analytics.current.marketplaces.slice(0, 5).map(compactMetric),
    categories: analytics.current.categories.slice(0, 5).map(compactMetric),
    skus: analytics.current.skus.slice(0, 5).map(compactMetric),
    ads: analytics.current.ads.slice(0, 5).map(compactMetric),
    goals: {
      source: 'Forecast da base carregada',
      totals: analytics.current.forecastTotals,
      marketplaces: analytics.current.forecastMarketplaces.slice(0, 5).map(compactMetric),
      categories: analytics.current.forecastCategories.slice(0, 5).map(compactMetric)
    },
    abc: analytics.abc,
    ...
    const openAiResponse = await fetch('https://api.openai.com/v1/responses', {
      ...
      body: JSON.stringify({
        ...
        instructions: [
          ...
          'Para publicidade, interprete TACOS, ACOS, ROAS e MOAS somente quando estiverem disponíveis.',
  ...
  const openAiResponse = await fetch('https://api.openai.com/v1/responses', {
    ...
    body: JSON.stringify({
      ...
      instructions: [
        ...
        'Evite jargoes desnecessarios. Quando usar termos como TACOS, ROAS, ACOS ou MOAS, explique brevemente o significado no contexto.',
